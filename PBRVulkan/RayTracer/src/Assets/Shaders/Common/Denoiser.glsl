...
vec4 denoise(ivec2 coords) 
{
	...
	if (iter % 2 == 0)
	{	
        cval = imageLoad(InputImage, ivec2(fragCoord)).rgb;
	}
	else
	{
		cval = imageLoad(OutputImage, ivec2(fragCoord)).rgb;
	}	
    ...
    for(uint i = 0; i < 25; ++i)
    {
        ...
        if (iter % 2 == 0)
        {
            ctmp = imageLoad(InputImage, uv).rgb;
        }
        else
        {
	        ctmp = imageLoad(OutputImage, uv).rgb;
	    }
